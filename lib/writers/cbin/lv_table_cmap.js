...
class LvCmap extends Cmap {
  ...
  toCBin(ptr_size) {    
    ...
    let total_data_offset = cmaps_buf.length;
    ...
    for (let [ format, codepoints ] of subtables_plan) {
      ...
      if (format === 'format0_tiny') {
        ...
        glyph_ptr = u.balign4(Buffer.from(d));
      ...
      writePTR(has_charcodes?total_data_offset:0, ofs); ofs+=ptr_size; total_data_offset+=unicode_ptr.length;
      writePTR(has_ids?total_data_offset:0, ofs); ofs+=ptr_size; total_data_offset+=glyph_ptr.length;
